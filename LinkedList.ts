...
export class LinkedList<T>
    ...
    insertAt(index: number, data: T)
        ...
        if (index === 0 || index === -this._length) { this.unshift(data); }
            ...
            let current: LLNode<T> = this._travelToNode(index);
    ...
    private _findNodeMatches(comparator: (data: T) => boolean)
        ...
        for (let current: LLNode<T> | undefined = this._head;
                current; current = current.next)
            { if (comparator(current.data)) { nodeMatches.push(current); } }
    ...
    private _travelToNode(index: number)
        ...
        if (index < this._length / 2) {
            current = this._head!;
            for (let currentIndex: number = 0;
                    currentIndex < index; currentIndex++)
                { current = current.next!; }
        } else {
            current = this._tail!;
            for (let currentIndex: number = this._length - 1;
                    currentIndex > index ; currentIndex--)
